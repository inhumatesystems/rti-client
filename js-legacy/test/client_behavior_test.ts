...
test("publish subscribe", async () => {
    ...
    message.setPause(new RTI.proto.Empty())
...
test("set state publishes state", async () => {
    let received = false
    rti.subscribe(RTI.channel.control, RTI.proto.RuntimeControl, (message: RTI.proto.RuntimeControl) => {
        if (message.state && message.state.clientId == rti.clientId) received = true
    })
...
test("participant registration for client - sets participant", async () => {
    ...
    expect(rti.participant).toBe("mr.foo")
    expect(rti2.knownClients.find((c) => c.id == rti.clientId)?.getParticipant()).toBe("mr.foo")
})
...
test("ephemeral channel registered after first use - updates to ephemeral", async () => {
    ...
    rti2.subscribeText("ephem", () => {
        received = true
        ephemeral = rti2.knownChannels.find((c) => c.getName() == "ephem")!.getEphemeral()
    })
...
test("verify own token - works", async () => {
    const result = await rti.verifyToken(rti.authToken!)
